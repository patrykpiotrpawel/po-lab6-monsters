...
namespace Catch {
    namespace Benchmark {
        namespace Detail {
            ...
            const char *optimized_away_error::what() const
...
namespace Catch {
    ...
    const char *GeneratorException::what() const
...
namespace Catch {
    namespace Detail {
        ...
        bool CaseInsensitiveLess::operator()(StringRef
        lhs,
        StringRef rhs
        ) const {
        return
...
#    pragma GCC diagnostic ignored "-Wmissing-field-initializers"
    ...
    sigaction(signalDefs[i]
    .id, &oldSigActions[i], nullptr);
}
...
namespace Catch {
    namespace {
        ...
        void listTags(IEventListener &reporter, IConfig const &config) {
            auto const &testSpec = config.testSpec();
            std::vector <TestCaseHandle> matchedTestCases = filterTests(getAllTestCasesSorted(config), testSpec,
                                                                        config);
...
namespace Catch {
    ...
    namespace Detail {
        ...
        while ( separatorPos<reporterSpec.

        size()

        ) {
        const auto nextSeparator = findNextSeparator(separatorPos);
        parts.push_back( static_cast
        <std::string>( reporterSpec
        .
        substr(
...
,
m_includeSuccessfulResults( m_config
->
...
line &&file
!= other.
file &&(std::strcmp(file, other.file)
< 0));
}
...
auto StringRef::operator==(StringRef other) const

noexcept -> bool {
return m_size == other.
m_size
        &&(std::memcmp(m_start, other.m_start, m_size)
== 0);
}

bool StringRef::operator<(StringRef rhs) const
...
namespace Catch {
    ...
    void
            enforceNoDuplicateTestCases(std::vector < TestCaseHandle >
    const& tests ) {
...
namespace Catch {
    ReporterBase::ReporterBase(ReporterConfig &&config) :
            ...
            m_wrapped_stream(CATCH_MOVE(config).

    takeStream()

    ),
    m_stream( m_wrapped_stream
    ->
...
namespace Catch {
    namespace {
        ...
        // Colour::LightGrey
        static constexpr Colour::Code
        compactDimColour = Colour::FileName;
...
#else
        static constexpr Catch::StringRef
        compactFailedString = "failed"_sr;
        static constexpr Catch::StringRef
        compactPassedString = "passed"_sr;
#endif
...
namespace Catch {
    ...
    std::string serializeFilters(std::vector < std::string >
    const& filters ) {
...
namespace Catch {
    ...
    namespace {
        ...
        std::string fileNameTag(std::vector < Tag >
        const& tags) {
...
namespace Catch {
    ...
    namespace {
        ...
        tapFailedString = "not ok"_sr;
        static constexpr StringRef
        tapPassedString = "ok"_sr;
        static constexpr Colour::Code
        tapDimColour = Colour::FileName;
